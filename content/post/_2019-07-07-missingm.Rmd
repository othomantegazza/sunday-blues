...
dat %>% 
  ...
  drop_na() %>% 
  uncount(weights = total_dead_and_missing) %>% 
  ggplot(aes(x = region_of_incident,
             y = reported_date)) +
  ggbeeswarm::geom_quasirandom(size = .1, colour = purple, alpha = .2) +
  coord_flip() +
  theme_minimal()
...
dat %>% 
  ggplot(aes(x = long,
             y = lat)) +
  borders(fill = "#EAEA9F", colour = "white") +
  geom_point(aes(alpha = total_dead_and_missing), colour = "#CD0A39")+
  coord_map(projection = "mollweide" , orientation = c(90, 0, 0)) +
  guides(size = FALSE) +
  scale_alpha(range = c(.01, 1)) +
  theme_void()

# not sure of how to deal with n and h
dat %>% 
  select(lat, long, total_dead_and_missing) %>% 
  drop_na() %>% 
  uncount(weights = total_dead_and_missing) %>% 
  ggplot(aes(x = long,
             y = lat)) +
...
dat %>% 
  ...
  ggplot(aes(x = long,
             y = lat)) +
  borders(fill = "#EAEA9F", colour = "white") +
  stat_bin2d(bins = c(600, 250), alpha = .7) +
  # stat_density2d(aes(fill = stat(level),
  #                     geom = "polygon")) +
  scale_fill_viridis_c() +
  coord_map(projection = "mollweide" , orientation = c(90, 0, 0)) +
...
#   
# 
# ddd <- MASS::kde2d(x = tst$lat, y = tst$long)
# 
# tst %>% 
#   mutate(z = ddd$z %>% as.numeric())
...
#            crs = "+proj=longlat +datum=WGS84 +no_defs") %>% 
#   ggplot() +
#   geom_sf(data = wrld_simpl %>% st_as_sf(),
#           colour = "white",
#           fill = "#EAEA9F") +
#   geom_sf(colour = "#CD0A39",
#           size = .5,
#           alpha = .7) +
...
leaflet() %>% 
  addTiles() %>% 
  addMarkers(data = latlongs,
             clusterOptions = markerClusterOptions())

leaflet() %>% 
  addTiles() %>% 
  addCircles(data = latlongs)
