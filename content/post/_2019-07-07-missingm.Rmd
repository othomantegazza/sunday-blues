...
mm <- read_csv("_data/MissingMigrants-Global-2019-06-26T00-37-45.csv",
...
# dat$url[1:5] %>% str_split(pattern = ", ") %>% flatten_chr() %>% longurl::expand_urls()
...
dat_med <- 
  dat %>% 
  filter(region_of_incident %in% c("Mediterranean", "Europe"))
...
dat_africa <- 
  dat %>% 
  filter(region_of_incident %in% c("North Africa", "Sub-Saharan Africa",
                                   "Horn of Africa")) 

event_lab <-
  paste0("<strong>", "Number of dead or missing: ",
         dat_africa$total_dead_and_missing,
         "</strong>") %>% 
  lapply(htmltools::HTML)
...
```{r}
event_lab <-
  paste0("Number of dead or missing: ",
         dat$total_dead_and_missing, "<br>",
         "reference article: ",
         dat$url,
